// API Key Configuration
        ...
        // Message Management
        function addMessage(text, isUser = false, isError = false, prompt = '') {
            ...
            if (isUser) {
                ...
                if (text.startsWith('data:image/')) {
                    ...
                    refineBtn.innerHTML = '<i class="fas fa-sync-alt"></i> Refine';
                    refineBtn.onclick = () => sendMessage(prompt, 'refine');
                    actions.appendChild(refineBtn);

                    const editBtn = document.createElement('button');
                    editBtn.innerHTML = '<i class="fas fa-edit"></i> Edit';
                    editBtn.onclick = () => {
                        const newPrompt = window.prompt("Enter a new prompt to edit the image:", prompt);
                        if (newPrompt) {
                            sendMessage(newPrompt, 'edit', text);
                        }
                    };
        ...
        async function populateModels() {
            ...
            const staticGeminiModels = [
                { id: 'gemini-2.5-pro', name: 'Gemini 2.5 Pro', description: 'Most powerful model', provider: 'gemini', isImageModel: false },
                { id: 'gemini-2.5-flash', name: 'Gemini 2.5 Flash', description: 'Fast and efficient', provider: 'gemini', isImageModel: false },
                { id: 'gemini-2.0-flash', name: 'Gemini 2.0 Flash', description: 'Legacy fast model', provider: 'gemini', isImageModel: false },
                { id: 'gemini-2.5-flash-image-preview', name: 'Gemini 2.5 Flash Image', description: 'Conversational image generation', provider: 'gemini', isImageModel: true },
                { id: 'imagen-4.0-generate-001', name: 'Imagen 4.0', description: 'Dedicated image generation', provider: 'gemini', isImageModel: true }
            ];
            ...
            if (OPENROUTER_API_KEY) {
                try {
                    ...
                    const mappedModels = openRouterModels.map(model => ({
                        ...
                        provider: 'openrouter',
                        isImageModel: model.output_modalities.includes('image')
                    }));
        ...
        async function callImagen4API(prompt) {
            ...
            try {
                ...
                if (data.predictions && data.predictions[0]?.image?.bytesBase64Encoded) {
                    return `data:image/png;base64,${data.predictions[0].image.bytesBase64Encoded}`;
                }
        ...
        async function callOpenRouterImageAPI(model, prompt) {
        ...
        async function callGeminiFlashImageAPI(prompt, image = null) {
            ...
            try {
                ...
                if (imagePart) {
                    return `data:image/png;base64,${imagePart.inline_data.data}`;
                }
                throw new Error('No image data found in Gemini Flash response.');
            } catch (error) {
                console.error('Gemini Flash Image API Error:', error);
                throw error;
            }
        }
        ...
        async function sendMessage(message, action = 'send', imageData = null) {
            const prompt = (action === 'send') ? userInput.value.trim() : message;
            ...
            const onChunk = (text) => {
                ...
                if (aiMessageElement) {
                    aiMessageElement.innerHTML = formatAIResponse(currentResponse);
                }
                scrollToBottom();
            };
            ...
            try {
                if (currentMode === 'webSearch') {
                    ...
                    try {
                        ...
                        if (searchResults.length > 0) {
                            ...
                            addMessage("No web search results found. Trying a normal response.", false);
                            aiMessageElement = document.createElement('div');
                            aiMessageElement.classList.add('message', 'ai-message');
                            chatMessages.appendChild(aiMessageElement);
                            showTypingIndicator();
                    ...
                    try {
                        ...
                        if (modelData.provider === 'openrouter') {
                            imageUrl = await callOpenRouterImageAPI(currentModel, prompt);
                        } else if (currentModel === 'imagen-4.0-generate-001') {
                            imageUrl = await callImagen4API(prompt);
                        } else { // Default to Gemini Flash Image
                            imageUrl = await callGeminiFlashImageAPI(prompt, action === 'edit' ? imageData : null);
                        }

                        addMessage(imageUrl, false, false, prompt);
                        onComplete(imageUrl);
                    } catch (error) {
                        addMessage(`Image generation failed: ${error.message}`, false, true);
                        onComplete(""); // End generation
                    } finally {
                        hideThinkingIndicator();
                    }
                ...
                onComplete(""); // End the generation process
            }
        }
        ...
        function setupEventListeners() {
            sendButton.addEventListener('click', () => sendMessage(userInput.value, 'send'));

            userInput.addEventListener('keypress', function(e) {
                if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    sendMessage(userInput.value, 'send');
                }
            });
